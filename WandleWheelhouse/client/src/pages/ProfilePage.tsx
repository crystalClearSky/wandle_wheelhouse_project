...
const ProfilePage: React.FC = () => {
  ...
  const getCroppedImg = async (imageSrc: string, pixelCrop: Area, brightness: number, contrast: number): Promise<Blob | null> => {
  ...
  const handleAvatarUpload = async (event: FormEvent) => {
    ...
    try {
      ...
      if (isEditing && croppedAreaPixels) {
        ...
        if (croppedImage) fileToUpload = new File([croppedImage], selectedFile.name, { type: 'image/jpeg' });
      }
  ...
  if (!user) return <div className="text-center p-10 text-gray-600">Please log in to view your profile. <Link to="/" className="font-semibold text-indigo-600 hover:text-indigo-700 transition-colors duration-200">Go Home</Link></div>;
  ...
  return (
    <div className="container mx-auto px-6 py-12 max-w-3xl space-y-8">
      {/* Page Header */}
      <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-6 text-center">Your Profile</h1>
          ...
          <form onSubmit={handleAvatarUpload} className="w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-3">
                <label htmlFor="avatar-upload" className="block text-sm font-medium text-gray-700">Change picture (JPG, PNG, GIF - Max 5MB):</label>
                <input
                  id="avatar-upload"
                  type="file"
              ...
              <div className="flex flex-col gap-3">
                ...
                {uploadError && (
                  <div className="bg-red-50 border border-red-400 text-red-700 px-4 py-2 rounded-lg flex items-center gap-2" role="alert">
                    <ExclamationCircleIcon className="h-5 w-5 text-red-500" />
                    <span>{uploadError}</span>
                  </div>
                )}
                {uploadSuccess && (
                  <div className="bg-green-50 border border-green-400 text-green-700 px-4 py-2 rounded-lg flex items-center gap-2" role="alert">
            ...
            {selectedFile && isEditing && (
              <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
                <div className="bg-white rounded-xl p-6 w-full max-w-2xl relative">
                  <h3 className="text-xl font-semibold text-gray-800 mb-4">Edit Profile Picture</h3>
                  <div className="relative w-full h-96 mb-4" style={{ filter: `brightness(${brightness}%) contrast(${contrast}%)` }}>
                    <Cropper
      ...
      <section className="bg-white rounded-xl shadow-lg p-6 sm:p-8 hover:shadow-xl transition-shadow duration-300">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Update Details</h2>
        {profileUpdateError && (
          <div className="bg-red-50 border border-red-400 text-red-700 px-4 py-2 rounded-lg flex items-center gap-2 mb-4" role="alert">
            <ExclamationCircleIcon className="h-5 w-5 text-red-500" />
            <span>{profileUpdateError}</span>
          </div>
        )}
        {profileUpdateSuccess && (
          <div className="bg-green-50 border border-green-400 text-green-700 px-4 py-2 rounded-lg flex items-center gap-2 mb-4" role="alert">
            <CheckCircleIcon className="h-5 w-5 text-green-500" />
      ...
      <section className="bg-white rounded-xl shadow-lg p-6 sm:p-8 hover:shadow-xl transition-shadow duration-300">
        <h2 className="text-2xl font-semibold text-gray-800 mb-3">Manage Subscription</h2>
      ...
      <section className="bg-red-50 rounded-xl shadow-lg p-6 sm:p-8 border border-red-200 hover:shadow-xl transition-shadow duration-300">
        <h2 className="text-2xl font-semibold text-red-800 mb-3">Delete Account</h2>
        ...
        {deleteError && (
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded-lg flex items-center gap-2 mb-4" role="alert">
            <ExclamationCircleIcon className="h-5 w-5 text-red-500" />
